...
static NSString* model_file_name = @"mem_graph";
...
static NSString* labels_file_name = @"food101_labels";
...
const std::string output_layer_name = "Softmax";
      ...
      for (int index = 0; index < predictions.size(); index += 1) {
        ...
        if (predictionValue > 0.05f) {
          ...
          NSNumber *valueObject = [NSNumber numberWithFloat:predictionValue];
            LOG(ERROR) << label << ": " << predictionValue;
...
- (void)setPredictionValues:(NSDictionary *)newValues {
    const float decayValue = 0.0f; //0.75f;
    const float updateValue = 1.0f; //0.25f;
    const float minimumThreshold = 0.2f; //0.01f;
